    ...
    module.exports.syncLoop(vout.length, function (loop) {
      ...
      if (vout[i].scriptPubKey.type != 'nonstandard' && vout[i].scriptPubKey.type != 'nulldata') { 
        ...
        if(!vout[i].scriptPubKey.addresses) {
          spk_addr = vout[i].scriptPubKey.address;
          // console.log(spk_addr);
        } else {
          spk_addr = vout[i].scriptPubKey.addresses[0];
        }

        module.exports.is_unique(arr_vout, spk_addr, function(unique, index) {
          if (unique == true) {
            // unique vout
            module.exports.convert_to_satoshi(parseFloat(vout[i].value), function(amount_sat){
              arr_vout.push({addresses: spk_addr, amount: amount_sat});
              loop.next();
            });
          } else {
            // already exists
